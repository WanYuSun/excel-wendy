...
打包命令:
    pyinstaller --onefile --name excel_operator_week excel_operator_week.py

用法:
    双击运行可执行文件或: python excel_operator_week.py
"""
...
def handle_entry(entry_name: str, entry_dir: str, excels: List[str],
    ...
    调用指定入口名的处理函数
    """
    log_stage("入口处理", f"开始处理周结入口: {entry_name}")

    handler = None
    matched_key = None

    # 首先尝试精确匹配
    if entry_name in week_entry_registry:
        handler = week_entry_registry[entry_name]
        matched_key = entry_name
    else:
        # 如果精确匹配失败，尝试startswith匹配
        for handler_key in week_entry_registry:
            if entry_name.startswith(handler_key):
                handler = week_entry_registry[handler_key]
                matched_key = handler_key
                break

    if not handler:
        log_warning(f"未注册周结入口处理器: '{entry_name}'")
        return

    log_info(f"[{entry_name}] 使用周结处理器: {matched_key}")

    try:
        handler(entry_dir, excels, conn)
        log_success(f"[{entry_name}] 周结入口处理完成")
    except Exception as e:
        log_error(f"[{entry_name}] 周结入口处理失败: {e}")
        raise
...
def main():
    ...
    # 设置统一的日志配置，在确定工作目录后
    log_file_path = os.path.join(base_dir, "excel_week.log")
    logger = setup_logging(log_file=log_file_path)
    
    log_stage("程序启动", "Excel周结数据处理程序开始运行")
    ...
    db_path = os.path.join(base_dir, "excel_week.db")
    ...
    try:
        ...
        # 处理所有入口
        log_stage("批量处理", f"开始周结处理 {len(entries)} 个入口")
        processed_count = 0
        failed_count = 0
        ...
        for i, entry_dir in enumerate(entries, 1):
            try:
                ...
                processed_count += 1
                log_success(f"入口 {entry_name} 周结处理完成")
            except Exception as e:
                failed_count += 1
                log_error(f"入口 {os.path.basename(entry_dir)} 周结处理失败: {e}")
                continue

        # 处理结果汇总
        log_stage("处理完成", f"所有周结入口处理完毕")
