...
def kuaishou_month_entry_handler(entry_dir: str, excels: List[str],
    ...
    try:
        ...
        kuaishou_matches = [x for x in excels if re.search(
            kuaishou_pattern, x, re.IGNORECASE)]
        ...
        if len(kuaishou_matches) == 1:
            ...
            if choice in ['y', 'yes', '是']:
                kuaishou_excels = [os.path.join(
                    entry_dir, f) for f in kuaishou_matches]
            else:
                # 让用户选择单个文件
    ...
    try:
        # 处理第一个文件
        log_info(f"[{entry_name}] 开始处理第一个文件: {kuaishou_excels[0]}")
        ...
        for i, file in enumerate(kuaishou_excels[1:], 2):
            ...
            try:
                ...
                temp_projection_name, _ = safe_process_excel_file(
                    file, temp_table, [
                        (used_projections, used_projection_name)], entry_name
                )
        ...
        try:
            ...
            log_info(
                f"[{entry_name}] 文件处理汇总: 成功 {successful_files} 个，失败 {failed_files} 个")
...
"""
                ...
                execute_sql_with_timing(
                    conn, export_sql, f"[{entry_name}] 导出第{sheet_num + 1}个临时sheet")
            ...
            for i, temp_file in enumerate(temp_files, 1):
                ...
                if os.path.exists(temp_file):
                    try:
                        ...
                        log_success(
                            f"[{entry_name}] 文件已重命名: {os.path.basename(final_name)}")
                    except:
                        log_warning(f"[{entry_name}] 无法重命名文件: {temp_file}")

            log_success(
                f"[{entry_name}] 快手月结数据已分离到 {sheets_needed} 个Excel文件，总计 {final_row_count} 行数据")
...
"""
            execute_sql_with_timing(
                conn, export_sql, f"[{entry_name}] 导出快手月结数据")
