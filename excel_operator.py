...
- 对每个子目录，收集所有`.xlsx`文件。
- 提供入口处理接口，可自定义处理逻辑。
- 注册“广点通”入口，查找匹配的Excel文件并执行DuckDB SQL。
- 全过程中文日志提示。
- 兼容Windows和Linux。
...
from excel.handlers.week.guangdiantong import guangdiantong_entry_handler
from excel.handlers.week.guangdiantong_v2 import guangdiantong_v2_entry_handler
from excel.handlers.week.kuaishou import kuaishou_entry_handler
from excel.handlers.week.toutiao import toutiao_entry_handler
# 导入统一的日志模块
...
entry_registry: Dict[str, EntryHandler] = {}


def register_entry(name: str, handler: EntryHandler):
    """
    注册入口处理函数
    """
    entry_registry[name] = handler
...
def handle_entry(entry_name: str, entry_dir: str, excels: List[str],
    ...
    """
    log_stage("入口处理", f"开始处理入口: {entry_name}")
    ...
    if not handler:
        log_warning(f"未注册入口处理器: '{entry_name}'")
        return

    log_info(f"[{entry_name}] 使用处理器: {matched_key}")

    try:
        handler(entry_dir, excels, conn)
        log_success(f"[{entry_name}] 入口处理完成")
    except Exception as e:
        log_error(f"[{entry_name}] 入口处理失败: {e}")
        raise


# 注册“广点通”入口处理器
register_entry("广点通", guangdiantong_entry_handler)
register_entry("广点通-大端口", guangdiantong_v2_entry_handler)
# 注册“快手”入口处理器
register_entry("快手", kuaishou_entry_handler)
# 注册“头条”入口处理器
register_entry("头条", toutiao_entry_handler)
...
def main():
    ...
    - 获取当前工作目录
    - 列出所有子目录
    - 优先处理“广点通”入口（account表需先加载）
    - 对每个子目录，列出Excel文件并用同名入口处理
    """
    ...
    db_path = os.path.join(os.getcwd(), "excel.db")
    ...
    try:
        ...
        # 处理所有入口
        log_stage("批量处理", f"开始处理 {len(entries)} 个入口")
        processed_count = 0
        failed_count = 0
        ...
        for i, entry_dir in enumerate(entries, 1):
            try:
                ...
                processed_count += 1
                log_success(f"入口 {entry_name} 处理完成")
            except Exception as e:
                failed_count += 1
                log_error(f"入口 {os.path.basename(entry_dir)} 处理失败: {e}")
                continue

        # 处理结果汇总
        log_stage("处理完成", "所有入口处理完毕")
